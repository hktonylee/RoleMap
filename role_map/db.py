...
def _migrate_jobs(connection: sqlite3.Connection) -> None:
    ...
    if "is_read" not in columns:
        connection.execute(
            "ALTER TABLE jobs ADD COLUMN is_read INTEGER NOT NULL DEFAULT 0"
        )
    ...
    if "created" not in columns:
        connection.execute("ALTER TABLE jobs ADD COLUMN created TEXT")
        connection.execute(
            """
            UPDATE jobs
            SET created = COALESCE(created_at, last_update)
