...
class CodexSkillTest(unittest.TestCase):
    def test_add_job_description_skill_exists_and_documents_workflow(self) -> None:
        ...
        self.assertIn("not a generated summary", body)
        self.assertIn("not a combined description", body)
        self.assertIn("copy 100% of the actual job description section", body)
        self.assertIn("Markdown conversion is formatting only", body)
        self.assertIn("Do not invent, condense, paraphrase, reorder, or merge", body)
        self.assertIn("never use an email snippet as the final description", body)
